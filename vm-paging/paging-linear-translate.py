...
parser.add_option('-A', '--addresses', default='-1',
                  help='一组逗号分隔的页面访问；-1表示随机生成', 
                  action='store', type='string', dest='addresses')
parser.add_option('-s', '--seed',    default=0,     help='随机种子',                               action='store', type='int', dest='seed')
parser.add_option('-a', '--asize',   default='16k', help='地址空间大小（例如16、64k、32m、1g）',   action='store', type='string', dest='asize')
parser.add_option('-p', '--physmem', default='64k', help='物理内存大小（例如16、64k、32m、1g）', action='store', type='string', dest='psize')
parser.add_option('-P', '--pagesize', default='4k', help='页面大小（例如4k、8k等）',            action='store', type='string', dest='pagesize')
parser.add_option('-n', '--numaddrs',  default=5,  help='生成的虚拟地址数',       action='store', type='int', dest='num')
parser.add_option('-u', '--used',       default=50, help='虚拟地址空间的使用百分比', action='store', type='int', dest='used')
parser.add_option('-v',                             help='详细模式',                                  action='store_true', default=False, dest='verbose')
