...
parser.add_option('-a', '--addr',            default='-1',        help='请求列表（逗号分隔）[-1 -> 使用addrDesc]',     action='store', type='string', dest='addr')
parser.add_option('-A', '--addrDesc',        default='5,-1,0',    help='请求数、最大请求（-1->全部）、最小请求',        action='store', type='string', dest='addrDesc')
...
parser.add_option('-w', '--schedWindow',     default=-1,          help='调度窗口大小（-1 -> 全部）',                   action='store', type='int',    dest='window')
parser.add_option('-o', '--skewOffset',      default=0,           help='偏斜量（以块为单位）',                              action='store', type='int',    dest='skew')
parser.add_option('-z', '--zoning',          default='30,30,30',  help='外、中、内磁道上块之间的角度',      action='store', type='string', dest='zoning')
parser.add_option('-G', '--graphics',        default=False,       help='启用图形',                                        action='store_true',           dest='graphics')
parser.add_option('-l', '--lateAddr',        default='-1',        help='延迟：请求列表（逗号分隔）[-1 -> 随机]',     action='store', type='string', dest='lateAddr')
parser.add_option('-L', '--lateAddrDesc',    default='0,-1,0',    help='请求数、最大请求（-1->全部）、最小请求',        action='store', type='string', dest='lateAddrDesc')
