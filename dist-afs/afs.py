...
parser.add_option('-C', '--clients',   default=2,      help='客户端数量',         action='store', type='int', dest='numclients')
parser.add_option('-n', '--numsteps',  default=2,      help='每个客户端要执行的操作',   action='store', type='int', dest='numsteps')
parser.add_option('-f', '--numfiles',  default=1,      help='服务器中的文件数', action='store', type='int', dest='numfiles')
parser.add_option('-r', '--readratio', default=0.5,    help='读写比例',     action='store', type='float', dest='readratio')
parser.add_option('-A', '--actions',   default='',     help='精确指定客户端操作，例如oa1:r1:c1,oa1:w1:c1指定两个客户端；每个都打开文件a，客户端0读取它而客户端1写入它，然后每个都关闭它', action='store', type='string', dest='actions')
parser.add_option('-S', '--schedule',  default='',     help='要运行的精确调度；01在客户端0和1之间交替循环轮询。未指定则进行随机调度', action='store', type='string', dest='schedule')
parser.add_option('-p', '--printstats', default=False, help='打印额外的统计信息',      action='store_true', dest='printstats')
parser.add_option('-c', '--compute',    default=False, help='为我计算答案', action='store_true', dest='solve')
parser.add_option('-d', '--detail',     default=0,     help='给出答案时的详细级别（1：服务器操作，2：失效，4：客户端缓存，8：额外标签）；OR多个组合', action='store', type='int', dest='detail')
