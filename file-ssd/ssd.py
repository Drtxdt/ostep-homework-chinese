...
parser.add_option('-n', '--num_cmds',        default=10,         help='要随机生成的命令数', action='store', type='int',    dest='num_cmds')
parser.add_option('-P', '--op_percentages',  default='40/50/10', help='如果是随机，读/写/修剪的百分比',  action='store', type='string', dest='op_percentages')
parser.add_option('-K', '--skew',            default='',         help='如果非空，偏斜，例如80/20：80%的操作到20%的块', action='store', type='string', dest='skew')
parser.add_option('-k', '--skew_start',      default=0,          help='如果--skew，在这么多次写入后偏斜',  action='store', type='int',    dest='skew_start')
parser.add_option('-r', '--read_fails',      default=0,          help='如果是随机，可能失败的读取百分比', action='store', type='int',    dest='read_fail')
parser.add_option('-L', '--cmd_list',        default='',         help='逗号分隔的命令列表（例如r10,w20:a）', action='store', type='string', dest='cmd_list')
parser.add_option('-T', '--ssd_type',        default='direct',   help='SSD类型：ideal, direct, log',            action='store', type='string', dest='ssd_type')
parser.add_option('-l', '--logical_pages',   default=50,         help='接口中的逻辑页面数',    action='store', type='int',    dest='num_logical_pages')
parser.add_option('-B', '--num_blocks',      default=7,          help='SSD中的物理块数',        action='store', type='int',    dest='num_blocks')
parser.add_option('-p', '--pages_per_block', default=10,         help='每个物理块的页面数',                action='store', type='int',    dest='pages_per_block')
parser.add_option('-G', '--high_water_mark', default=10,         help='gc触发前使用的块数',           action='store', type='int',    dest='high_water_mark')
parser.add_option('-g', '--low_water_mark',  default=8,          help='停止gc前的gc目标',            action='store', type='int',    dest='low_water_mark')
parser.add_option('-R', '--read_time',       default=10,         help='页面读取时间（usecs）',                  action='store', type='int',    dest='read_time')
parser.add_option('-W', '--program_time',    default=40,         help='页面编程时间（usecs）',               action='store', type='int',    dest='program_time')
parser.add_option('-E', '--erase_time',      default=1000,       help='页面擦除时间（usecs）',                 action='store', type='int',    dest='erase_time')
parser.add_option('-J', '--show_gc',         default=False,      help='显示垃圾收集器行为',         action='store_true',           dest='show_gc')
parser.add_option('-F', '--show_state',      default=False,      help='显示闪存状态',                        action='store_true',           dest='show_state')
parser.add_option('-C', '--show_cmds',       default=False,      help='显示命令',                           action='store_true',           dest='show_cmds')
parser.add_option('-q', '--quiz_cmds',       default=False,      help='测试命令',                           action='store_true',           dest='quiz_cmds')
parser.add_option('-S', '--show_stats',      default=False,      help='显示统计信息',                         action='store_true',           dest='show_stats')
