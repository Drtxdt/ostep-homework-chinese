...
class scheduler:
    ...
    def load(self, program_description):
        ...
        if len(tmp) != 2:
            print('错误的描述 (%s)：必须是数字 <x:y>' % program_description)
            print('  其中X是指令数')
            print('  Y是指令是CPU而不是IO的百分比')
            exit(1)
...
parser.add_option('-P', '--program', default='', help='对程序的更多具体控制', action='store', type='string', dest='program')
parser.add_option('-l', '--processlist', default='', help='逗号分隔的进程列表，格式为X1:Y1,X2:Y2,...，其中X是进程应运行的指令数，Y是指令使用CPU或发出IO的机率（0到100），即如果Y是100，进程将仅使用CPU且不发出IO；如果Y是0，进程将仅发出IO', action='store', type='string', dest='process_list')
parser.add_option('-L', '--iolength', default=5, help='IO需要多长时间', action='store', type='int', dest='io_length')
parser.add_option('-S', '--switch', default='SWITCH_ON_IO', help='何时在进程之间切换：SWITCH_ON_IO、SWITCH_ON_END', action='store', type='string', dest='process_switch_behavior')
parser.add_option('-I', '--iodone', default='IO_RUN_LATER', help='IO结束时的行为类型：IO_RUN_LATER、IO_RUN_IMMEDIATE', action='store', type='string', dest='io_done_behavior')
parser.add_option('-c', help='为我计算答案', action='store_true', default=False, dest='solve')
parser.add_option('-p', '--printstats', help='在结束时打印统计信息；仅对-c标志有用（否则不打印统计信息）', action='store_true', default=False, dest='print_stats')
...
if options.print_stats:
    print('')
    print('统计信息：总时间 %d' % clock_tick)
    print('统计信息：CPU忙碌 %d (%.2f%%)' % (cpu_busy, 100.0 * float(cpu_busy)/clock_tick))
    print('统计信息：IO忙碌  %d (%.2f%%)' % (io_busy, 100.0 * float(io_busy)/clock_tick))
    print('')
