...
parser.add_option('-m', '--mapping',         default='0',         help='0-条纹化，1-镜像，4-raid4，5-raid5',               action='store', type='int',    dest='mapping')
parser.add_option('-a', '--addr',            default='',          help='请求列表（逗号分隔）[-1 -> 使用addrDesc]',     action='store', type='string', dest='addr')
parser.add_option('-r', '--read_fraction',   default='0.5',       help='读取请求的比例',                     action='store', type='string', dest='read_fraction')
parser.add_option('-A', '--addr_desc',       default='5,-1,0',    help='请求数、最大请求（-1->全部）、最小请求',        action='store', type='string', dest='addr_desc')
parser.add_option('-B', '--balanced',        default=True,        help='如果生成随机请求，则跨磁盘均衡',     action='store_true',           dest='balance')
parser.add_option('-S', '--seek_speed',      default='4',         help='寻道速度（1,2,4,5,10,20）',                           action='store', type='int',    dest='seek_speed')
parser.add_option('-p', '--policy',          default='FIFO',      help='调度策略（FIFO, SSTF, SATF, BSATF）',             action='store', type='string', dest='policy')
parser.add_option('-w', '--window',          default=-1,          help='调度窗口大小（-1 -> 全部）',                   action='store', type='int',    dest='window')
parser.add_option('-D', '--delay',           default=20,          help='动画延迟；越大越慢',                       action='store', type='int',    dest='animate_delay')
parser.add_option('-G', '--graphics',        default=True,        help='启用图形',                                        action='store_true',           dest='graphics')
parser.add_option('-c', '--compute',         default=False,       help='为我计算答案',                                     action='store_true',           dest='compute')
parser.add_option('-P', '--print_options',   default=False,       help='打印选项',                                       action='store_true',           dest='print_options')
