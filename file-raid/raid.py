...
parser.add_option('-D', '--numDisks',    default=4,      help='RAID中的磁盘数',                        action='store',       type='int',    dest='numDisks') 
parser.add_option('-C', '--chunkSize',   default='4k',   help='RAID的块大小',                         action='store',       type='string', dest='chunkSize') 
parser.add_option('-n', '--numRequests', default=10,     help='要模拟的请求数',                 action='store',       type='int',    dest='numRequests')
parser.add_option('-S', '--reqSize',     default='4k',   help='请求的大小',                               action='store',       type='string', dest='size')
parser.add_option('-W', '--workload',    default='rand', help='"rand"或"seq"工作负载',               action='store',       type='string', dest='workload')
parser.add_option('-w', '--writeFrac',   default=0,      help='写入比例（100->全部写入，0->全部读取）', action='store',       type='int',    dest='writeFrac')
parser.add_option('-R', '--randRange',   default=10000,  help='请求范围（使用"rand"工作负载时）', action='store',       type='int',    dest='range')
parser.add_option('-L', '--level',       default=0,      help='RAID级别（0, 1, 4, 5）',                        action='store',       type='int',    dest='level')
parser.add_option('-5', '--raid5',       default='LS',   help='RAID-5左对称"LS"或左非对称"LA"',   action='store',       type='string', dest='raid5type')
parser.add_option('-r', '--reverse',     default=False,  help='显示物理操作而不是逻辑操作',  action='store_true',                 dest='reverse')
parser.add_option('-t', '--timing',      default=False,  help='使用计时模式，而不是映射模式',       action='store_true',                 dest='timing')
