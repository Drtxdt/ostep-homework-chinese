...
class Forker:
    ...
    def walk(self, p, level, pmask, is_last):
        ...
        if self.print_style == 'basic':
            ...
            print('错误的样式 %s' % self.print_style)
            exit(1)
    ...
    def bad_action(self, action):
        print('错误的操作 (%s)，必须是X+Y或X-，其中X和Y是进程' % action)
        exit(1)
        return
        ...
        if self.action_list != '':
            ...
            while actions < self.max_actions:
                if random.random() < self.fork_percentage:
                    ...
                    #        exception: no killing root processs, sorry
        ...
        for a in action_list:
            ...
            if len(tmp) == 2:
                ...
                if self.leaf_only and len(self.children[exit_choice]) > 0:
                    action = '%s 退出 (失败：有子进程)' % exit_choice
                else:
...
parser.add_option('-f', '--forks', default=0.7, help='是fork的行为百分比（不是exit）', action='store', type='float', dest='fork_percentage')
parser.add_option('-A', '--action_list', default='', help='操作列表，而不是随机生成的（格式：a+b,b+c,b-代表a fork b、b fork c、b exit）', action='store', type='string', dest='action_list')
parser.add_option('-a', '--actions', default=5, help='要执行的fork/exit数量', action='store', type='int', dest='actions')
parser.add_option('-t', '--show_tree', help='显示树状图（不显示操作）', action='store_true', default=False, dest='show_tree')
parser.add_option('-P', '--print_style', help='树的打印样式（basic, line1, line2, fancy）', action='store', type='string', default='fancy', dest='print_style')
parser.add_option('-F', '--final_only', help='仅显示最终状态', action='store_true', default=False, dest='just_final')
parser.add_option('-L', '--leaf_only', help='仅叶进程退出', action='store_true', default=False, dest='leaf_only')
parser.add_option('-R', '--local_reparent', help='重新分配给本地父进程', action='store_true', default=False, dest='local_reparent')
