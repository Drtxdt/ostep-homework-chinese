...
parser.add_option('-s', '--seed',        default=0,     help='第一个随机种子（用于文件系统）', action='store', type='int', dest='seed')
parser.add_option('-S', '--seedCorrupt', default=0,     help='第二个随机种子（用于损坏）', action='store', type='int', dest='seedCorrupt')
parser.add_option('-i', '--numInodes',   default=16,    help='文件系统中的inode数',      action='store', type='int', dest='numInodes') 
parser.add_option('-d', '--numData',     default=16,    help='文件系统中的数据块数', action='store', type='int', dest='numData') 
parser.add_option('-n', '--numRequests', default=15,    help='要模拟的请求数',       action='store', type='int', dest='numRequests')
parser.add_option('-p', '--printFinal',  default=False, help='打印最终的文件/目录集',    action='store_true',        dest='printFinal')
parser.add_option('-w', '--whichCorrupt',default=-1,    help='执行特定的损坏',             action='store', type='int', dest='whichCorrupt')
parser.add_option('-c', '--compute',     default=False, help='为我计算答案',               action='store_true',        dest='solve')
parser.add_option('-D', '--dontCorrupt', default=False,  help='实际上损坏文件系统',        action='store_true',        dest='dontCorrupt')
