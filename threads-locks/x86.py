...
parser.add_option('-t', '--threads',   default=2,          help='线程数',                action='store',      type='int',    dest='numthreads')
parser.add_option('-p', '--program',   default='',         help='源程序（.s格式）',           action='store',      type='string', dest='progfile')
parser.add_option('-i', '--interrupt', default=50,         help='中断频率',              action='store',      type='int',    dest='intfreq')
parser.add_option('-P', '--procsched', default='',         help='精确控制哪个线程何时运行',
                                                                                                    action='store',      type='string', dest='procsched')
parser.add_option('-r', '--randints',  default=False,      help='如果中断是随机的',         action='store_true',                dest='intrand')
parser.add_option('-a', '--argv',      default='',
                  help='以逗号分隔的每线程参数（例如ax=1,ax=2将线程0的ax寄存器设置为1，线程1的ax寄存器设置为2）；通过冒号分隔的列表为每个线程指定多个寄存器（例如ax=1:bx=2,cx=3为线程0设置ax和bx，仅为线程1设置cx）',
                  action='store',      type='string', dest='argv')
parser.add_option('-L', '--loadaddr',  default=1000,       help='代码加载地址',       action='store',      type='int',    dest='loadaddr')
parser.add_option('-m', '--memsize',   default=128,        help='地址空间大小（KB）',       action='store',      type='int',    dest='memsize')
parser.add_option('-M', '--memtrace',  default='',         help='要跟踪的地址的逗号分隔列表（例如20000,20001）', action='store',
                  type='string', dest='memtrace')
parser.add_option('-R', '--regtrace',  default='',         help='要跟踪的寄存器的逗号分隔列表（例如ax,bx,cx,dx）',  action='store',
                  type='string', dest='regtrace')
parser.add_option('-C', '--cctrace',   default=False,      help='我们是否应该跟踪条件代码',  action='store_true', dest='cctrace')
parser.add_option('-S', '--printstats',default=False,      help='打印一些额外的统计信息',           action='store_true', dest='printstats')
parser.add_option('-v', '--verbose',   default=False,      help='打印一些额外的信息',            action='store_true', dest='verbose')
parser.add_option('-H', '--headercount',default=-1,        help='多久打印一次行标题',  action='store',      type='int',    dest='headercount')
