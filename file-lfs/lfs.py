...
parser.add_option('-N', '--no_force', help='不强制在更新后写入检查点', default=False, action='store_true', dest='no_force_checkpoints')
parser.add_option('-F', '--no_final', help='不显示文件系统的最终状态', default=False, action='store_true', dest='no_final')
parser.add_option('-D', '--use_disk_cr', help='使用磁盘（可能是旧版本）的检查点区域', default=False, action='store_true', dest='use_disk_cr')
parser.add_option('-c', '--compute', help='为我计算答案', action='store_true', default=False, dest='solve')
parser.add_option('-o', '--show_operations', help='按发生时打印操作', action='store_true', default=False, dest='show_operations')
parser.add_option('-i', '--show_intermediate', help='按发生时打印状态更改', action='store_true', default=False, dest='show_intermediate')
parser.add_option('-e', '--show_return_codes', help='显示错误/返回代码', action='store_true', default=False, dest='show_return_codes')
parser.add_option('-v', '--show_live_paths', help='显示实时路径', action='store_true', default=False, dest='show_live_paths')
parser.add_option('-n', '--num_commands', help='生成N个随机命令', action='store', default=3, dest='num_commands')
parser.add_option('-p', '--percentages', help='百分比概率：createfile,writefile,createdir,rmfile,linkfile,sync（示例为c30,w30,d10,r20,l10,s0）', action='store', default='c30,w30,d10,r20,l10,s0', dest='percentages')
parser.add_option('-a', '--allocation_policy', help='inode分配策略："r"表示"随机"或"s"表示"顺序"', action='store', default='s', dest='inode_policy')
parser.add_option('-L', '--command_list', default = '', action='store', type='str', dest='command_list', help='命令列表，格式为："cmd1,arg1,...,argN:cmd2,arg1,...,argN:...其中cmd为：c:createfile, d:createdir, r:delete, w:write, l:link, s:sync格式：c,filepath d,dirpath r,filepath w,filepath,offset,numblks l,srcpath,dstpath s')
