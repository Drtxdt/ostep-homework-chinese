...
parser.add_option('-P', '--program', default='', help='对程序的更具体控制', action='store', type='string', dest='program')
parser.add_option('-l', '--processlist', default='', help='运行进程的逗号分隔列表，格式为X1:Y1,X2:Y2,...，其中X是该进程应运行的指令数，Y是指令将使用CPU或发出IO的概率（0到100）', action='store', type='string', dest='process_list')
parser.add_option('-L', '--iolength', default=5, help='IO需要多长时间', action='store', type='int', dest='io_length')
parser.add_option('-o', '--interrupt_overhead', default=5, help='中断处理需要多长时间', action='store', type='int', dest='interrupt_overhead')
...
parser.add_option('-p', '--printstats', help='在结尾打印统计信息；仅在使用-c标志时有用（否则不打印统计信息）', action='store_true', default=False, dest='print_stats')
