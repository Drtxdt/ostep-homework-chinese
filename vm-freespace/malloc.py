...
parser.add_option('-b', '--baseAddr',    default=1000,       help='堆的基地址',                        action='store', type='int',    dest='baseAddr') 
parser.add_option('-H', '--headerSize',  default=0,          help='头的大小',                          action='store', type='int',    dest='headerSize')
parser.add_option('-a', '--alignment',   default=-1,         help='将已分配的单元对齐到大小；-1->无对齐', action='store', type='int',    dest='alignment')
parser.add_option('-p', '--policy',      default='BEST',     help='列表搜索（BEST、WORST、FIRST）',            action='store', type='string', dest='policy') 
parser.add_option('-l', '--listOrder',   default='ADDRSORT', help='列表顺序（ADDRSORT、SIZESORT+、SIZESORT-、INSERT-FRONT、INSERT-BACK）', action='store', type='string', dest='order') 
parser.add_option('-C', '--coalesce',    default=False,      help='合并释放列表？',                     action='store_true',           dest='coalesce')
parser.add_option('-n', '--numOps',      default=10,         help='生成的随机操作数',            action='store', type='int',    dest='opsNum')
parser.add_option('-r', '--range',       default=10,         help='最大分配大小',                              action='store', type='int',    dest='opsRange')
parser.add_option('-P', '--percentAlloc',default=50,         help='是分配的操作百分比',              action='store', type='int',    dest='opsPAlloc')
parser.add_option('-A', '--allocList',   default='',         help='而不是随机，操作列表（+10,-0等）', action='store', type='string', dest='opsList')
