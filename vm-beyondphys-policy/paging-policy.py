...
#
parser = OptionParser()
parser.add_option('-a', '--addresses', default='-1',   help='逗号分隔的页面访问集合；-1表示随机生成',  action='store', type='string', dest='addresses')
parser.add_option('-f', '--addressfile', default='',   help='包含一堆地址的文件',                                action='store', type='string', dest='addressfile')
parser.add_option('-n', '--numaddrs', default='10',    help='如果-a（--addresses）是-1，这是要生成的地址数',    action='store', type='string', dest='numaddrs')
parser.add_option('-p', '--policy', default='FIFO',    help='替换策略：FIFO、LRU、OPT、UNOPT、RAND、CLOCK',                action='store', type='string', dest='policy')
parser.add_option('-b', '--clockbits', default=2,      help='对于CLOCK策略，要使用多少个时钟位',                          action='store', type='int', dest='clockbits')
parser.add_option('-C', '--cachesize', default='3',    help='页面缓存的大小（以页面为单位）',                                      action='store', type='string', dest='cachesize')
parser.add_option('-m', '--maxpage', default='10',     help='如果随机生成页面访问，这是最大页面号',     action='store', type='string', dest='maxpage')
parser.add_option('-s', '--seed', default='0',         help='随机数种子',                                                    action='store', type='string', dest='seed')
parser.add_option('-N', '--notrace', default=False,    help='不打印详细跟踪',                                     action='store_true', dest='notrace')
