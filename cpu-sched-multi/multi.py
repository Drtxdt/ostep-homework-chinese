...
parser.add_option('-j', '--job_num',     default=3,     help='系统中的作业数',           action='store', type='int', dest='job_num')
parser.add_option('-R', '--max_run',     default=100,   help='随机生成作业的最大运行时间',        action='store', type='int', dest='max_run')
parser.add_option('-W', '--max_wset',    default=200,   help='随机生成作业的最大工作集',     action='store', type='int', dest='max_wset')
parser.add_option('-L', '--job_list',    default='',    help='提供逗号分隔的job_name:run_time:working_set_size列表（例如a:10:100,b:10:50表示2个作业，运行时间为10，第一个(a)的工作集大小=100，第二个(b)的工作集大小=50）', action='store', type='string', dest='job_list')
parser.add_option('-p', '--per_cpu_queues', default=False, help='每个CPU的调度队列（不是一个）', action='store_true',        dest='per_cpu_queues')
parser.add_option('-A', '--affinity',    default='',    help='作业和它们可以运行的CPU列表（例如a:0.1.2,b:0.1允许作业a在CPU 0,1,2上运行，但b仅在CPU 0和1上运行', action='store', type='string', dest='affinity')
parser.add_option('-n', '--num_cpus',    default=2,     help='CPU数量',                         action='store', type='int', dest='num_cpus')
parser.add_option('-q', '--quantum',     default=10,    help='时间片长度',                   action='store', type='int', dest='time_slice')
parser.add_option('-P', '--peek_interval', default=30,  help='对于每个CPU的调度，多久查看一次其他调度队列；0关闭此功能', action='store', type='int', dest='peek_interval')
parser.add_option('-w', '--warmup_time', default=10,    help='预热缓存需要的时间',            action='store', type='int', dest='warmup_time')
parser.add_option('-r', '--warm_rate', default=2,     help='使用预热缓存运行速度快多少', action='store', type='int', dest='warm_rate')
parser.add_option('-M', '--cache_size',  default=100,   help='缓存大小',                             action='store', type='int', dest='cache_size')
parser.add_option('-o', '--rand_order',  default=False, help='CPU以随机顺序获取作业',      action='store_true',        dest='random_order')
parser.add_option('-t', '--trace',       default=False, help='启用基本跟踪（显示哪些作业被调度）',      action='store_true',        dest='trace')
parser.add_option('-T', '--trace_time_left', default=False, help='跟踪每个作业的剩余时间',       action='store_true',        dest='trace_time_left')
parser.add_option('-C', '--trace_cache', default=False, help='也跟踪缓存状态（预热/冷）',     action='store_true',        dest='trace_cache')
parser.add_option('-S', '--trace_sched', default=False, help='跟踪调度器状态',                  action='store_true',        dest='trace_sched')
